...
def make_workflow_csv(slack_channel_messages, channel_id, TS_YESTERDAY,TS_TODAY):
    ...
    if not "feedback.csv" in box_items[ROOT_FOLDER_NAME]['items'][channel_folder_name]['items'][date_folder_name]['items'].keys():
        if feedbacklist:
...
def file_upload_slack2box(file_ids):
    ...
    for file in file_ids:
        ...
        if not box_items:
            ...
            logger.info(f"No root folder in Box")

        elif not channel_name in box_items[ROOT_FOLDER_NAME]["items"].keys():
            box_items = get_items_from_box_folder(channel_folder_name=channel_name,date_folder_name=date_folder_name,root_folder_name=ROOT_FOLDER_NAME)
            logger.info(f'No channel folder {channel_name} under the root folder')

        #date_folder_nameの存在確認を行う
        elif not date_folder_name in box_items[ROOT_FOLDER_NAME]["items"][channel_name]["items"].keys():
            #存在しなければ、取得もしくは作成を行う
            box_items = get_items_from_box_folder(channel_folder_name=channel_name,date_folder_name=date_folder_name,root_folder_name=ROOT_FOLDER_NAME)
            logger.info(f'No date folder {date_folder_name} under the channel folder')
